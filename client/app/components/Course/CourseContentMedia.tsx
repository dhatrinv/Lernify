...
import { SerializedError } from '@reduxjs/toolkit'; // Correct import for SerializedError
...
const CourseContentMedia = ({
  ...
  const [
    addNewQuestion,
    { isSuccess, error: questionError, isLoading: questionCreationLoading },
  ] = useAddNewQuestionMutation();
  ...
  const [questionToAsk, setQuestionToAsk] = useState<string>('');
  const [aiAnswer, setAiAnswer] = useState<string | null>(null);
  const [aiErrorState, setAiErrorState] = useState<string | null>(null); // Renamed state variable to aiErrorState

  const [askAI, { isLoading: aiLoading, error }] = useAskAiMutation(); // 'error' from the hook

  const handleAiQuestionSubmit = async () => {
    if (!questionToAsk) return;

    const contentId = data[activeVideo]._id; // Assuming activeVideo and data are defined
    ...
    try {
      ...
      setAiErrorState(null); // Clear any previous errors
    } catch (err) {
      console.error("Error while asking AI:", err);
      setAiErrorState("An error occurred while asking the AI.");
    }
  };
  ...
  return (
    <div className="w-[95%] 800px:w-[86%] py-4 m-auto">
      <CoursePlayer
        title={data[activeVideo]?.title || "No Title"}
      ...
      <h1 className="pt-2 text-[25px] font-[600] dark:text-white text-black ">
        {data[activeVideo]?.title || "No Title"}
      </h1>
      <br />
      <div className="w-full p-4 flex items-center justify-between bg-slate-500 bg-opacity-20 backdrop-blur shadow-[bg-slate-700] rounded shadow-inner">
        {["Overview", "Resources", "Q&A", "Reviews", "LearniFy AI"].map(
          (text, index) => (
            <h5
              key={index}
      ...
      {activeBar === 4 && (
        ...
        <div>
          <h3 className="text-2xl mb-4 text-center font-semibold dark:text-white text-black">
            Ask LearniFy AI
          </h3>
          <textarea
            className="w-full p-2 mb-3 border border-[#ccc] rounded dark:bg-[#2d2d2d] dark:text-white"
            rows={4}
            placeholder="Ask your question..."
            value={questionToAsk}
            onChange={(e) => setQuestionToAsk(e.target.value)}
          />
          <button
        ...
        <div className="mt-6">
          ...
          {aiAnswer ? (
            <div>
              ...
              <div className="bg-gray-100 p-4 rounded-md dark:bg-[#444] dark:text-white">
                <div className="mb-3">
                  <p>{aiAnswer}</p>
                </div>
              </div>
            ...
            <p className="text-center text-gray-500 dark:text-white">
              No answer yet. Ask your question above!
            </p>
          )}
        </div>
      </div>
